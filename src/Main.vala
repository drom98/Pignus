...
public class Main {
     ...
     *
     * @see Pignus.Application
     * @return {@code int}
     * @since 1.0.0
     */
    public static int main (string [] args) {
        var options_context = new OptionContext (Pignus.Configs.Constants.PROGRAME_NAME +" "+ _("Options"));
        ...
        if (version) {
            stdout.printf (Pignus.Configs.Constants.PROGRAME_NAME +" "+ Pignus.Configs.Constants.VERSION + "\r\n");
            return 0;
        }

        if (testing) {
            Gtk.init (ref args);
            var testing = new Pignus.Tests.Testing (args);
            Idle.add (() => {
                testing.run ();
                Gtk.main_quit ();
            ...
            var app = new Pignus.Application ();
            app.run (args);
        }
